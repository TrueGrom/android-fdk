 ...
 *
 * Every slot here is optional: [Loading], [Error], [AppendLoading] and [AppendError] fall back to
 * the current [LocalPagingDefaults] — the same instance for both layouts — and [Empty] renders
...
interface FdkPagingSlotsBuilder {
    ...
    /**
     * Refresh error content, filling the viewport where one is known (see [Loading]); defaults to
     * a message with a retry button.
     *
     ...
     * the app surfaces it.
     */
    fun Error(content: @Composable FdkPagingSlotScope.(e: Throwable, retry: () -> Unit) -> Unit)
     ...
     *
     * [e] is the failure the append ended on, worded like [Error]'s.
     */
...
internal class PagingSlots(
    ...
    val error: @Composable FdkPagingSlotScope.(e: Throwable, retry: () -> Unit) -> Unit,
...
internal abstract class PagingSlotsBuilderImpl : FdkPagingSlotsBuilder {
    ...
    private var error: @Composable FdkPagingSlotScope.(e: Throwable, retry: () -> Unit) -> Unit =
        { e, retry -> with(LocalPagingDefaults.current) { Error(e, retry) } }
    ...
    final override fun Error(content: @Composable FdkPagingSlotScope.(e: Throwable, retry: () -> Unit) -> Unit) { error = content }
    ...
    protected fun buildSlots() =
        PagingSlots(loading, error, empty, appendLoading, appendError, transition)
}
...
internal fun <T : Any> emitPagingSlots(
    ...
    items.ifEmpty {
        ...
        // layout's arrangement would space it away from whatever a Prepend put above it.
    ...
    items.expand(
        ...
        error = {
            ...
            slot("paging_slot_error", false) {
                val e = items.refreshErrorOrNull() ?: emitted
                if (e != null) slots.error(this, e) { items.retry() }
            }
        },
    )
}
